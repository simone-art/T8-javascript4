...
class card {
  ...
  desenhaCard() {
    ...
    cardDiv.setAttribute("class", "avatar green")
    const cardImg = document.createElement("img")
    cardImg.src = this.imagem
    cardImg.setAttribute("class", img-rounded)
    const cardTitle = document.createElement('h2')
    cardTitle.innerHTML = this.usuario.nome
    cardDiv.appendChild(cardImg)
    cardDiv.appendChild(cardTitle)
    document.getElementById('cards-section').appendChild(cardDiv)
    // console.log(cardImg)
